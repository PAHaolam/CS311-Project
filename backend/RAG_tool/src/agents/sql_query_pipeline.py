...
class SQLQueryPipeline:
    def __init__(self):
        ...
        self.response_synthesis_prompt_str = (
            ...
            "1. if users are finding some specific books and given retrieved rows includes the books, you will define whether if there is only one book match the query or there are many books match the query\n"
            "1.1. If there is only one book match the query, your response should be:\n"
            "Vâng, bên mình có cuốn sách (or truyện depending on Query) ...(Name of books that user expect), bạn có thể tham khảo qua!\n"
            "1.2. If there are many books match the query, your response should be:\n"
            "Đây là danh sách sách các sách (or truyện depending on Query) liên quan đến ..(Name of books that user expect), mời bạn tham khảo!\n"
            ...
            "\n"
            "Then you will define whether the user wants to display books in order such as quantity sold, price, day of publication, ... or not.\n"
            "1. If user do not want to display books in order, please skip\n"
            "2. If user want to display books in order, please add some sentences to your response like:\n"
            "Mình đã sắp xếp các sách (or truyện depending on Query) theo tứ tự ... (order that user expect)"
            "\n"
            "Then you can ask the user some other questions that the user might be interested in related to the question.\n"
            "Please note that your response including any information from the books in given retrieved rows is invalid.\n"
            "Example:\n"
            ...
            "\n"
            "Query: Cho tôi danh sách các truyện Naruto mà bạn có theo giá từ thấp đến cao"
            "Response: Bên mình truyện Naruto hiện tại đang có rất nhiều, bạn có thể xem qua, đồng thời mình đã sắp xếp các sách theo thứ tự giá thấp đến cao cho bạn tham khảo rồi nhé!"
            "\n"
